...
/**
 * 图片处理(第一步,独立操作):读黑白图 → 二值化(DocScannerBinarize)→ JBIG压缩(DocScannerJBIG)
 * → 产物写成 .txt 到 /storage/emulated/0/ceshi/。复刻旧 App 真实流程(去掉矫正 correction)。
 *
...
public class ImageProcessor {
    ...
    private boolean ensureInit() {
        if (binarize == null) {
            cb.log("INFO", "初始化二值化/JBIG 引擎(首次加载 opencv/onnx 稍慢)…");
    ...
    private void runBatch(List<File> imgs) {
        ...
        for (int i = 0; i < imgs.size() && !stop; i++) {
            ...
            try {
                ...
                Bitmap bin = binarize.binarize(src);
                src.recycle();
