...
public final class Protocol {
     ...
     *   LH:LL = 6 + ENTRY_SIZE*N(指 Byte3=cmd 到帧尾 0xAA 的字节数; 与 buildData/buildCmd 一致)
    ...
    public static byte[] buildImageInfo(int batchId, int[] sizes, int[] crc16s) {
        ...
        int lenField = 6 + es * n;                       // = 从Byte3数到帧尾的字节数(与buildData/buildCmd一致;=帧尾下标-2)
    ...
    /**
     * 目录文件 dir.bin 的内容 = 0x07 **载荷**(不含 0x39 信封):
     *   batch_id(1) N(1) entry_size(1) + N×{bytes4 pkts2 crc16_2}
     * App-0.9.6 起: 目录改成"落地文件 + 当文件传"(作为本批最后一项, 图号=img_total-1),
     * 不再用空中 0x07 帧。N = 真实图片张数(不含目录自身)。
     */
    public static byte[] buildDirectoryPayload(int batchId, int[] sizes, int[] crc16s) {
        int n = sizes.length;
        int es = IMAGE_INFO_ENTRY_SIZE;
        byte[] p = new byte[3 + es * n];
        int q = 0;
        p[q++] = (byte) (batchId & 0xFF);
        p[q++] = (byte) (n & 0xFF);
        p[q++] = (byte) (es & 0xFF);
        for (int i = 0; i < n; i++) {
            int bytes = sizes[i];
            int pkts = (bytes + PACKET_SIZE - 1) / PACKET_SIZE;   // ceil(bytes/190)
            int crc = crc16s[i] & 0xFFFF;
            p[q++] = (byte) ((bytes >> 24) & 0xFF);
            p[q++] = (byte) ((bytes >> 16) & 0xFF);
            p[q++] = (byte) ((bytes >> 8) & 0xFF);
            p[q++] = (byte) (bytes & 0xFF);
            p[q++] = (byte) ((pkts >> 8) & 0xFF);
            p[q++] = (byte) (pkts & 0xFF);
            p[q++] = (byte) ((crc >> 8) & 0xFF);
            p[q++] = (byte) (crc & 0xFF);
        }
