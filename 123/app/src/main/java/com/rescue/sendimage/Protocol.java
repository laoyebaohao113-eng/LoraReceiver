 ...
 *
 * 图片信息帧(命令 0x07),可变长 7+2N 字节(N=图片张数):
 *   39  LH:LL  07  N  S1H S1L ... SNH SNL  CHK  AA
 *   LH:LL = 4 + 2N(指 Byte3 到最后数据字节的索引)
 *   Si = 第i张图片(txt)字节数,16位大端
 *   CHK = sum(Byte3..最后数据字节)&0xFF —— 单字节(板子固件:仅0x01双字节,其余单字节)
 *   在 0x06 转换完成之后、第一包 0x01 数据之前发送;接收端用它做补包依据。
 */
public final class Protocol {
    ...
    /**
     * 拼图片信息帧(命令 0x07):告知接收端共多少张图片、每张图片字节数(补包依据)。
     * 帧:39  LH:LL  07  N  S1H S1L ... SNH SNL  CHK  AA,整帧 7+2N 字节。
     * 校验为单字节(板子固件:仅 0x01 双字节,其余单字节)。
     * @param sizes 每张图片(txt)字节数,顺序与发送顺序一致;单张>65535 会被截断到16位(上层应已告警)
     */
    public static byte[] buildImageInfo(int[] sizes) {
        int n = sizes.length;
        int lenField = 4 + 2 * n;            // 长度字段 = Byte3 到最后数据字节的索引
        byte[] f = new byte[7 + 2 * n];      // 索引 0 .. 6+2n
        f[0] = HEAD;
        f[1] = (byte) ((lenField >> 8) & 0xFF);
        f[2] = (byte) (lenField & 0xFF);
        f[3] = (byte) CMD_IMAGE_INFO;
        f[4] = (byte) (n & 0xFF);            // 图片总张数
        for (int i = 0; i < n; i++) {
            int s = sizes[i] & 0xFFFF;       // 16位
            f[5 + 2 * i] = (byte) ((s >> 8) & 0xFF);
            f[6 + 2 * i] = (byte) (s & 0xFF);
        }
        int sum = 0;
        for (int i = 3; i <= 4 + 2 * n; i++) sum += (f[i] & 0xFF);  // Byte3..最后数据字节
        f[5 + 2 * n] = (byte) (sum & 0xFF);  // CHK 单字节
        f[6 + 2 * n] = TAIL;
        return f;
    }
