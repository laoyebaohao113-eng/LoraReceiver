...
public class ImageProcessor {
    ...
    private void runBatch(List<File> imgs) {
        ...
        for (int i = 0; i < imgs.size() && !stop; i++) {
            ...
            // v0.9.5:处理前打点(写进日志文件)——万一 native 崩, 最后这条就是凶手图
            Runtime rt = Runtime.getRuntime();
            cb.log("PROC", "[" + (i + 1) + "/" + imgs.size() + "] " + img.getName()
                    + " start; file=" + (img.length() / 1024) + "KB"
                    + " freeMem=" + (rt.freeMemory() / 1048576) + "/" + (rt.maxMemory() / 1048576) + "M");
            try {
                ...
                cb.log("PROC", "  decoded " + src.getWidth() + "x" + src.getHeight()
                        + " (~" + ((long) src.getWidth() * src.getHeight() * 4 / 1048576) + "MB/bitmap; 矫正+二值各再分配一份等大int[])");
