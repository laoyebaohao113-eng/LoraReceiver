...
public class ImageSender implements Runnable {
    ...
    /** 控制命令之间的间隔(ms):母体对控制命令不回ACK,留点时间让板子处理状态机 */
    private static final int CTRL_GAP_MS = 100;
    ...
    public void run() {
        try {
            ...
            // 不发 0x07 图片信息帧!板子对非 0x01 帧用单字节校验,但比较时拿 16 位累加和(uint16)
            // 跟 1 个字节(uint8)比;0x07 装的是图片字节数,累加和必 >255 → 板子必判校验失败,
            // 发出通用错误帧 27 00 04 01 02 03,会被误当成"第0包数据失败"。板子又不解析 0x07 内容,
            // 发它有害无益,直接跳过,0x06 之后进第一包数据。
     ...
     * 故 fire-and-forget——只发不等 ACK,发完留 CTRL_GAP_MS 间隔让板子处理状态机。
