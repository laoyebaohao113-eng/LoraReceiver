...
public class ImageSender implements Runnable {
    ...
    private static int s_batchId = 0;   // 0x07 批序号(每发一批+1)
    ...
    public void run() {
        try {
            ...
            // 发 0x07 图片信息帧(0.9.3):A-1.2.2 已修 A 的非0x01校验bug、A-1.2.3 把0x07载荷转发给B、B-1.1.7 解析。
            // 富版:batch_id + N + entry_size + 每张{bytes4+pkts2+crc16_2}。APP侧重发3遍做简单可靠化。
            try {
                int n07 = files.size();
                int[] sizes07 = new int[n07];
                int[] crcs07 = new int[n07];
                for (int k = 0; k < n07; k++) {
                    byte[] d07 = TxtSource.readBytes(files.get(k));
                    sizes07[k] = d07.length;
                    crcs07[k] = Protocol.crc16Ccitt(d07, 0, d07.length);
                }
                int batchId07 = (s_batchId++) & 0xFF;
                byte[] info07 = Protocol.buildImageInfo(batchId07, sizes07, crcs07);
                for (int rep = 0; rep < 3 && !stop; rep++) {
                    tx.send(info07);
                    cb.log("TX", "[0x07图片信息 batch" + batchId07 + " N" + n07 + " len" + info07.length + "] 第" + (rep + 1) + "/3遍");
                    Thread.sleep(ctrlGapMs > 0 ? ctrlGapMs : 200);
                }
            } catch (Exception e07) {
                cb.log("ERR", "0x07发送异常: " + e07.getMessage());
            }
